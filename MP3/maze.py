...
class Maze(object):
    ...
    def show_estimated_location(self, particles):
        ...
        for particle in particles:
            ...
            head_cos_accum += np.cos(particle.heading) * particle.weight
            head_sin_accum += np.sin(particle.heading) * particle.weight

        if wgt_accum == 0:
            return False

        x_estimate = x_accum / wgt_accum
        y_estimate = y_accum / wgt_accum
        head_sin_accum = head_sin_accum / wgt_accum
        head_cos_accum = head_cos_accum / wgt_accum
        heading_estimate = np.arctan2(head_sin_accum, head_cos_accum) * 180 / np.pi
